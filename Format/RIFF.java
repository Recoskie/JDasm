...
public class RIFF extends Data implements JDEventListener
{
  ...
  public RIFF() throws java.io.IOException
  {
    ...
    if( initPaths.size() > 0 )
    {
      //If format header exists.

      tree.setSelectionPath( initPaths.get(0) ); open( new JDEvent( this, "", ((JDNode)tree.getLastSelectedPathComponent()).getArgs() ) );
    }

    //Else set the RIFF header as main header.

    else { tree.setSelectionPath( new TreePath( root.getFirstLeaf().getPath() ) ); open( new JDEvent( this, "", 0 ) ); }
  ...
  public static final String[] RIFFInfo = new String[]
  {
    ...
    "We need to know when we reached the end of the file otherwise we will be reading undefined sections after the files end.</html>",
